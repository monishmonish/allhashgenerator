...
def hashing(get_input):
    ...
    print(G+"{+}The MD4 Hash Value Of Your Input Is {+}: "+md4_hash)

    #md5
    md5_hash=hashlib.md5(encoded_input)
    print(G+"{+}The MD5 Hash Value Of Your Input Is {+}: "+md5_hash.hexdigest())

    #md5 crypt
    from passlib.hash import md5_crypt as mc
    md5_crypt = mc.encrypt(get_input)
    print (G+"{+}The MD5 Crypt Hash Value Of Your Input Is {+}: "+md5_crypt)

    #sun md5 crypt
    from passlib.hash import sun_md5_crypt as mc
    sunmd5_crypt = mc.encrypt(get_input)
    print (G+"{+}The Sun MD5 Crypt Hash Value Of Your Input Is {+}: "+sunmd5_crypt)

    #apache's md5 crypt
    from passlib.hash import apr_md5_crypt as mc
    apachemd5_crypt = mc.encrypt(get_input)
    print (G+"{+}The Apache MD5 Crypt Hash Value Of Your Input Is {+}: "+apachemd5_crypt)

    
    #sha1
    sha1_hash=hashlib.sha1(encoded_input)
    print(G+"{+}The SHA1 Hash Value Of Your Input Is {+}: "+sha1_hash.hexdigest())
    
    #sha224
    sha224_hash=hashlib.sha224(encoded_input)
    print(G+"{+}The SHA224 Hash Value Of Your Input Is {+}: "+sha224_hash.hexdigest())
    
    #sha256
    sha256_hash=hashlib.sha256(encoded_input)
    print(G+"{+}The SHA256 Hash Value Of Your Input Is {+}: "+sha256_hash.hexdigest())
    
    #sha384
    sha384_hash=hashlib.sha384(encoded_input)
    print(G+"{+}The SHA384 Hash Value Of Your Input Is {+}: "+sha384_hash.hexdigest())
    
    #sha512
    sha512_hash=hashlib.sha512(encoded_input)
    print(G+"{+}The SHA512 Hash Value Of Your Input Is {+}: "+sha512_hash.hexdigest())

    #sha1 Crypt
    from passlib.hash import sha1_crypt as mc
    sha1=mc.encrypt(get_input)
    print (G+"{+}The SHA1 Crypt Hash Value Of Your Input Is {+}: "+sha1)

    #sha256 Crypt
    from passlib.hash import sha256_crypt as mc
    sha256_crypt=mc.encrypt(get_input)
    print (G+"{+}The SHA256 Crypt Hash Value Of Your Input Is {+}: "+sha256_crypt)

    # Sha512 crypt
    from passlib.hash import sha512_crypt as mc
    sha512_crypt = mc.encrypt(get_input)
    print (G+"{+}The SHA512 Crypt Hash Value Of Your Input Is {+}: "+sha512_crypt)
    
    #django sha1
    from passlib.hash import django_pbkdf2_sha1 as m25
    django_sha1 = m25.encrypt(get_input)
    print (G+"{+}The Django HMAC SHA1 Hash Value Of Your Input Is {+}: "+django_sha1)

    #django sha256
    from passlib.hash import django_pbkdf2_sha256 as m25
    django_sha256 = m25.encrypt(get_input)
    print (G+"{+}The Django HMAC SHA256 Hash Value Of Your Input Is {+}: "+django_sha256)
    ...
    print (G+"{+}The Blake2b Hash Value Of Your Input Is {+}: "+blake2b_hash)
